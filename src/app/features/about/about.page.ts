    ...
    <section class="pt-32 pb-16 relative">
      ...
      <div class="max-w-7xl mx-auto px-6 relative z-10">
        <div class="flex items-center gap-3 mb-6">
          ...
          <span class="font-mono text-xs text-cool-400 tracking-[0.3em] uppercase">{{this.i18n.t().aboutPage.subtitle}}</span>
        </div>
    ...
    <section class="py-16">
      <div class="max-w-4xl mx-auto px-6">
        ...
        <div class="space-y-8 relative">
          <div class="absolute left-4.75 top-4 bottom-4 w-px bg-linear-to-b from-cool-500/30 via-cool-500/10 to-transparent hidden md:block"></div>
          @for (exp of data.experience(); track exp.period) {
            <div class="relative md:pl-14">
              <div class="hidden md:block absolute left-3.5 top-2 w-2.75 h-2.75 rounded-full border-2 border-cool-500 bg-surface-950"></div>
              <div class="p-8 rounded-2xl bg-surface-800/40 border border-white/5">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
                  ...
                  <span class="font-mono text-[10px] text-muted-400 tracking-[0.15em]">{{ exp.period }}</span>
                </div>
                <p class="text-cool-400/80 text-sm font-medium mb-3">{{ exp.company }}</p>
                <p class="text-white/40 text-sm leading-relaxed mb-4">{{ exp.description }}</p>
                <div class="flex flex-wrap gap-2">
                  @for (tag of exp.tags; track tag) {
                    <span class="font-mono text-[9px] px-2.5 py-1 rounded-full bg-cool-500/10 text-cool-400/70 border border-cool-500/10">
                      {{ tag }}
                    </span>
                  }
                </div>
