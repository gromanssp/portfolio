  ...
  template: `
    ...
    <section class="pt-32 pb-16 relative">
      ...
      <div class="max-w-7xl mx-auto px-6 relative z-10 text-center">
        <span class="font-mono text-xs text-accent-400 tracking-[0.3em] uppercase">{{this.i18n.t().contactPage.subtitle}}</span>
        <h1 class="font-display font-black text-4xl md:text-6xl text-white mt-4 mb-6">
          {{this.i18n.t().contactPage.title}} <span class="text-accent-400 glow-text">{{this.i18n.t().contactPage.titleHighlight}}</span>?
        </h1>
        <p class="text-white/40 text-lg max-w-xl mx-auto">
          {{this.i18n.t().contactPage.description}}
        </p>
      </div>
    </section>
        ...
        <div class="grid lg:grid-cols-5 gap-12">
          ...
          <div class="lg:col-span-3">
            <div class="p-10 rounded-3xl bg-surface-800/40 border border-white/5">
              @if (!submitted()) {
                <h3 class="font-display font-bold text-xl text-white mb-8">{{this.i18n.t().contactPage.formTitle}}</h3>
                <div class="space-y-6">
                  <div class="grid md:grid-cols-2 gap-6">
                    <div>
                      <label class="font-mono text-[10px] text-accent-400/60 tracking-[0.2em] uppercase block mb-2">{{this.i18n.t().contactPage.labelName}}</label>
                    ...
                    <div>
                      <label class="font-mono text-[10px] text-accent-400/60 tracking-[0.2em] uppercase block mb-2">{{this.i18n.t().contactPage.labelEmail}}</label>
                  ...
                  <div>
                    <label class="font-mono text-[10px] text-accent-400/60 tracking-[0.2em] uppercase block mb-2">{{this.i18n.t().contactPage.labelSubject}}</label>
                    <select class="input-cyber w-full px-4 py-3 rounded-xl text-sm text-white/50 font-body appearance-none cursor-pointer">
                      <option>{{this.i18n.t().contactPage.selectSubject}}</option>
                      <option>{{this.i18n.t().contactPage.subjectOptions[0]}}</option>
                      <option>{{this.i18n.t().contactPage.subjectOptions[1]}}</option>
                      <option>{{this.i18n.t().contactPage.subjectOptions[3]}}</option>
                      <option>{{this.i18n.t().contactPage.subjectOptions[4]}}</option>
                    </select>
                  </div>
                  <div>
                    <label class="font-mono text-[10px] text-accent-400/60 tracking-[0.2em] uppercase block mb-2">{{this.i18n.t().contactPage.labelMessage}}</label>
                    <textarea rows="5" placeholder="{{this.i18n.t().contactPage.placeholderMessage}}"
                  ...
                  <button (click)="send()" [disabled]="true"
                    ...
                    {{ sending() ? this.i18n.t().contactPage.sending : this.i18n.t().contactPage.send }}
                  </button>
                </div>
              } @else {
                <div class="text-center py-12" style="animation: fade-in 0.5s ease forwards">
                  ...
                  <h3 class="font-display font-bold text-2xl text-white mb-3">{{this.i18n.t().contactPage.successTitle}}</h3>
                  <p class="text-white/40 text-sm">{{this.i18n.t().contactPage.successDescription}}</p>
                </div>
              }
            </div>
          </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            <div class="p-8 rounded-2xl bg-surface-800/40 border border-white/5">
              <h4 class="font-display font-semibold text-white mb-6">{{this.i18n.t().contactPage.infoTitle}}</h4>
              <div class="space-y-5">
            ...
            <div class="p-8 rounded-2xl bg-surface-800/40 border border-white/5">
              <h4 class="font-display font-semibold text-white mb-4">{{this.i18n.t().contactPage.availabilityTitle}}</h4>
              <div class="space-y-3">
                <div class="flex justify-between">
                  <span class="text-white/40 text-sm">Freelance</span>
                  <span class="font-mono text-xs text-neon-green">{{this.i18n.t().contactPage.available}}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-white/40 text-sm">Full-time</span>
                  <span class="font-mono text-xs text-accent-400">{{this.i18n.t().contactPage.openToOffers}}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-white/40 text-sm">{{this.i18n.t().contactPage.remote}}</span>
                  <span class="font-mono text-xs text-neon-green">{{this.i18n.t().contactPage.preferred}}</span>
                </div>
              </div>
            </div>
...
export class ContactPage {
  ...
  contactInfo = [
    ...
    { label: this.data.infoLocation(), value: this.data.location, icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z' },
  ];
