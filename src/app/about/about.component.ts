...
export class AboutComponent implements OnDestroy {
  ...
  readonly bioScenes = signal<BioScene[]>([
    ...
    { year: '2025', techs: [
      ...
      { icon: 'fa-solid fa-brain', label: 'IA', color: '#9C27B0' }
    ], context: 'Automatización inteligente y agentes conversacionales', duration: 5500, milestone: true },
  ]);
  ...
  private startLoop(): void {
    const play = (i: number) => {
      if (i >= this.bioScenes().length) i = 0;
      this.currentSceneIndex.set(i);
      this.fadingSceneIndex.set(null);
      const scene = this.bioScenes()[i];
      const fadeOutStart = scene.duration - 400;
      this.sceneTimer = setTimeout(() => {
        this.fadingSceneIndex.set(i);
        this.sceneTimer = setTimeout(() => play(i + 1), 400);
      }, fadeOutStart);
    };
    play(0);
  }
